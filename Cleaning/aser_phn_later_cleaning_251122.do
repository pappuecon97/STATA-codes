...
*Created on: STATA17
*Last Modified on: 25/11/22
*Last Modified by: MJI
*Purpose : Basic cleaning of phone later dataset
*Edits 
...
*load dataset
use "$raw_data_dir\Validation of Phone Based ASER Assessment October_2022_ 6 Nov 2022_clean", clear
...
/*
There is an unexpected missing in english 1st qs (eq6_*), there is time for that qs, also the consent is available, but answer is missing. So, I first browse the id to know the main_id, then I checked the audio to correct for the answer. Fortunately, the child got 2 answer right (big and man), hence our skipping rule is valid for the alphabet qs. So, doing correction would be fine here. 
*/

*correct for this error 
*br if eq6_1 ==.

*replace value with correct one
forvalues i = 1/5{
	replace eq6_2 = 1 if exta_e1 ==1 & eq6_`i' ==. 
	replace eq6_4 = 1 if exta_e1 ==1 & eq6_`i' ==. 
	replace eq6_count = 2 if exta_e1 ==1 & eq6_`i' ==. 
}

foreach i in 1 3 5{
	replace eq6_`i' = 0 if exta_e1 ==1 & eq6_`i' ==. 
}

*check the correction
*br eq6* if psu ==553 & idno==11 & child_lino==3
///note: it seems okay
...
gen survey_mode = 4
la define surv_mod 4"phone later" 
...
format %28s saq7ot
format %33s saq11ot
format %8.0g child_consent_no


**********cleaning/incorporating others section from variables ***

recode saq6 9=11 if saq5ot_1==2 
/// recode as the child was ssc candidate 

recode saq7 555=.n if inlist(saq7ot_1, 13,14,15,16,17,18,20,21,22)
// recode as .n since sample info is incorrect 

recode saq7 555= 4 if inlist(saq7ot_1, 7,9)
...
replace saq11_25= 1 if inrange(saq11ot_1,1,6) | inlist(saq11ot_1, 16)
replace saq11_25= 0 if saq_11 !=. & saq11_25 !=1
la val saq11_25 yesnom

///rain/flood part of saq11_ot 
replace saq11_26= 0 if saq_11 !=. //no respondent choose this option 
la val saq11_26 yesnom

///take care of sick family members
replace saq11_9 = 1 if inlist(saq11ot_1, 20, 51)

///not mandatory to go to school every day 
replace saq11_20 = 1 if inlist(saq11ot_1, 14,15,23,24,27,29,32,33,34,35)

///financial problem 
replace saq12_2 = 1 if inlist(saq12ot_1, 1, 8)
...
translate @Results "$analysis_log_dir\missing_table_aser_phn_later.txt", replace
...
/*
Note: there are 2 children aged 11 and 14 study in class 1; 3 children aged 11 study in class 2.  
*/
...
*save cleaned data 
save "$clean_data_dir\aser_phn_later_cleaned.dta", replace 
